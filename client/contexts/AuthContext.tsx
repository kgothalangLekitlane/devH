...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { let cancelled = false; const restoreSession = async () => { try { const storedToken = localStorage.getItem('authToken'); if (!storedToken) return; const response = await fetchCurrentUser(storedToken); const currentUser = response?.user; if (!currentUser?.id) throw new Error('Invalid current-user response'); if (!cancelled) { setToken(storedToken); setUser(currentUser); localStorage.setItem('authUser', JSON.stringify(currentUser)); } } catch { if (!cancelled) { disconnectSocket(); setToken(null); setUser(null); clearStoredSession(); } } finally { if (!cancelled) setIsLoading(false); } }; restoreSession(); return () => { cancelled = true; }; }, []);
