    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[190px_minmax(0,1fr)]">
        ...
        <div className="space-y-6">
          ...
          <section id="notifications" className="rounded-3xl border border-border bg-card p-6"><h2 className="text-xl font-bold">Notifications</h2><p className="mt-1 text-sm text-muted-foreground">Choose which alerts you want to receive on your DevHeaven account.</p><div className="mt-5 space-y-3"><div className="flex items-center justify-between gap-4 rounded-2xl border border-border p-4"><div><p className="font-medium">New message emails</p><p className="text-sm text-muted-foreground">Receive an email when someone sends you a new DevHeaven message.</p></div><button type="button" disabled={notificationSaving} aria-pressed={messageEmailNotifications} onClick={updateMessageEmailNotifications} className={`relative h-6 w-11 rounded-full transition disabled:cursor-not-allowed disabled:opacity-60 ${messageEmailNotifications ? "bg-cyan-500" : "bg-muted"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${messageEmailNotifications ? "left-6" : "left-1"}`} /></button></div>{([ ["jobAlerts", "Job alerts", "Get reminders about relevant opportunities."], ["messageAlerts", "Message alerts", "Keep track of new conversations and connection activity."] ] as const).map(([key, title, description]) => <div key={key} className="flex items-center justify-between gap-4 rounded-2xl border border-border p-4"><div><p className="font-medium">{title}</p><p className="text-sm text-muted-foreground">{description}</p></div><button type="button" aria-pressed={preferences[key]} onClick={() => updatePreference(key)} className={`relative h-6 w-11 rounded-full transition ${preferences[key] ? "bg-cyan-500" : "bg-muted"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${preferences[key] ? "left-6" : "left-1"}`} /></button></div>)}</div></section>
