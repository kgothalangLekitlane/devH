...
import { fetchUserById, assetUrl, fetchConnections, requestConnection, updateConnection } from "@/lib/api"
import { formatLocalDateTime } from "@/lib/time"
import { useAuth } from "@/contexts/AuthContext"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Code2, MapPin, Calendar, MessageCircle, UserPlus, Check, X } from "lucide-react"
...
export default function UserProfilePage() {
  ...
  const handleRequest = async () => {
    ...
    setActionError("")
    try {
      const body = await requestConnection(String(user._id), token)
      setConnection(body.connection)
    } catch (err: any) {
      setActionError(err.message || "Unable to send connection request")
    } finally {
      setBusy(false)
    }
  }
  ...
  const handleDecision = async (status: "accepted" | "rejected") => {
    ...
    setActionError("")
    try {
      const body = await updateConnection(String(connection._id), status, token)
      setConnection(body.connection)
    } catch (err: any) {
      setActionError(err.message || "Unable to update connection")
    } finally {
      setBusy(false)
    }
  }
