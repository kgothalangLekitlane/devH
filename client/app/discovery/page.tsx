...
export default function DiscoveryPage() {
  ...
  const load = async () => {
    ...
    try {
      const params = { q: searchQuery.trim(), skill: filters.skill.trim(), location: filters.location.trim(), experience: filters.experience }
  ...
  useEffect(() => { if (!isLoading && user) void load() }, [isLoading, user, token, searchQuery, filters.skill, filters.location, filters.experience])

  const relation = (id: string) => connections.find(c => idOf(c.requester) === id || idOf(c.recipient) === id)
  const isSearching = Boolean(searchQuery.trim() || filters.skill.trim() || filters.location.trim() || filters.experience)
  const clearSearch = () => { setQuery(""); setSearchQuery(""); setSkill(""); setLocation(""); setExperience(""); setFilters({ skill: "", location: "", experience: "" }) }
  const applyFilters = () => setFilters({ skill: skill.trim(), location: location.trim(), experience })
  const incoming = connections.filter(c => c.status === "pending" && idOf(c.recipient) === idOf(user?.id))
  const outgoing = connections.filter(c => c.status === "pending" && idOf(c.requester) === idOf(user?.id))
  ...
  const connectionPeople = useMemo(() => accepted.map(c => {
    const requester = idOf(c.requester) === idOf(user?.id) ? c.recipient : c.requester
    return requester
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <main className="relative mx-auto max-w-7xl px-4 py-8">
      <section className="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-500/15 via-slate-900 to-violet-600/15 p-7 md:p-9"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs font-semibold text-cyan-300"><Compass className="h-3.5 w-3.5" /> Discover people</div><h1 className="text-3xl font-extrabold tracking-tight md:text-4xl">Find your people on DevHeaven.</h1><p className="mt-2 max-w-2xl text-slate-400">Search developers, explore profiles, discover shared skills and connect with people building interesting things.</p></div><div className="grid grid-cols-2 gap-2 text-center md:grid-cols-5"><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><p className="text-xl font-bold">{stats.connections}</p><p className="text-[11px] text-slate-500">Connections</p></div><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><p className="text-xl font-bold">{outgoing.length}</p><p className="text-[11px] text-slate-500">Sent</p></div><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><p className="text-xl font-bold">{incoming.length}</p><p className="text-[11px] text-slate-500">Received</p></div><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><p className="text-xl font-bold">{stats.pendingRequests}</p><p className="text-[11px] text-slate-500">Pending</p></div><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"><p className="text-xl font-bold">{stats.extendedNetwork}</p><p className="text-[11px] text-slate-500">Extended</p></div></div></div></section>
      ...
      {tab === "discover" && <>
        <Card className="mt-5 border-white/10 bg-white/[0.04]"><CardContent className="p-5"><form onSubmit={e => { e.preventDefault(); setSearchQuery(query.trim()) }} className="flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cyan-400" /><Input aria-label="Search people" className="h-12 border-white/10 bg-slate-950/60 pl-10 pr-4" placeholder="Search people by name, username or skill..." value={query} onChange={e => setQuery(e.target.value)} /></div><Button type="submit" className="h-12 shrink-0 bg-gradient-to-r from-cyan-500 to-violet-600 px-6 text-white"><Search className="mr-2 h-4 w-4" />Search</Button></form><div className="mt-3 flex items-center gap-2 text-xs text-slate-500"><SlidersHorizontal className="h-3.5 w-3.5" /> Refine your discovery</div><div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-3"><Input className="border-white/10 bg-slate-950/50" placeholder="Skill e.g. React" value={skill} onChange={e => setSkill(e.target.value)} /><Input className="border-white/10 bg-slate-950/50 !text-white placeholder:!text-slate-600" placeholder="Location" value={location} onChange={e => setLocation(e.target.value)} /><select className="h-10 rounded-md border border-white/10 bg-slate-950/50 px-3 text-sm text-slate-300" value={experience} onChange={e => setExperience(e.target.value)}><option value="">Any experience</option><option value="0">0 years</option><option value="1">1+ years</option><option value="3">3+ years</option><option value="5">5+ years</option><option value="10">10+ years</option></select></div><div className="mt-3 flex flex-wrap items-center gap-2"><Button type="button" onClick={applyFilters} variant="outline" className="border-white/10 bg-white/5 text-slate-200 hover:bg-white/10"><SlidersHorizontal className="mr-2 h-4 w-4" />Apply filters</Button>{isSearching && <Button type="button" onClick={clearSearch} variant="ghost" className="text-slate-400 hover:bg-white/5 hover:text-white">Clear</Button>}</div></CardContent></Card>
        {error && <div className="mt-4 rounded-xl border border-red-400/20 bg-red-500/10 p-3 text-sm text-red-300">{error}</div>}
        {!loading && isSearching && <div className="mt-5 flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-slate-400">Showing results{searchQuery ? ` for “${searchQuery}”` : ""}</p><p className="text-xs text-slate-500">{people.length} match{people.length === 1 ? "" : "es"}</p></div>}
        {loading ? <div className="py-16 text-center text-slate-500">Finding developers for you...</div> : people.length === 0 ? <Card className="mt-5 border-white/10 bg-white/[0.04]"><CardContent className="py-16 text-center"><Users className="mx-auto h-10 w-10 text-slate-600" /><h2 className="mt-3 font-semibold">{isSearching ? "No people found" : "No developers found"}</h2><p className="mt-1 text-sm text-slate-500">{isSearching ? "Try a different name, username, skill or filter." : "Try a broader search or update your profile."}</p>{isSearching && <Button type="button" onClick={clearSearch} variant="outline" className="mt-5 border-white/10 bg-white/5 text-slate-200 hover:bg-white/10">Clear search</Button>}</CardContent></Card> : <div className="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">{people.map(person => { const id = idOf(person); const r = relation(id); const pendingFromMe = r?.status === "pending" && idOf(r.requester) === idOf(user.id);
        const pendingToMe = r?.status === "pending" && idOf(r.recipient) === idOf(user.id); return <Card key={id} className="group border-white/10 bg-white/[0.04] transition-all hover:-translate-y-1 hover:border-cyan-400/20 hover:bg-white/[0.06]"><CardContent className="p-6"><div className="flex items-start justify-between"><Avatar className="h-16 w-16 border border-white/10"><AvatarImage src={assetUrl(person.profileImage)} /><AvatarFallback className="bg-gradient-to-br from-cyan-500 to-violet-600 text-white">{person.firstName?.[0]}{person.lastName?.[0]}</AvatarFallback></Avatar><div className="flex flex-col items-end gap-1">{person.sharedSkills > 0 && <Badge className="border-cyan-400/20 bg-cyan-400/10 text-cyan-300">{person.sharedSkills} shared skill{person.sharedSkills === 1 ? "" : "s"}</Badge>}{person.mutualConnections > 0 && <Badge variant="outline" className="border-white/10 text-slate-400">{person.mutualConnections} mutual</Badge>}</div></div><h3 className="mt-4 text-lg font-bold">{person.firstName} {person.lastName}</h3><p className="text-sm text-cyan-400">@{person.username || "developer"}</p>{person.headline && <p className="mt-1 line-clamp-1 text-sm text-slate-300">{person.headline}</p>}{person.location && <p className="mt-2 flex items-center gap-1 text-sm text-slate-500"><MapPin className="h-3.5 w-3.5" />{person.location}</p>}<p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-400">{person.bio || "Developer on DevHeaven"}</p><div className="mt-4 flex flex-wrap gap-1.5">{(person.skills || []).slice(0, 5).map((s: string) => <Badge key={s} variant="outline" className="border-white/10 text-xs text-slate-400">{s}</Badge>)}</div><div className="mt-5 flex gap-2"><Button asChild variant="outline" className="flex-1 border-white/10 bg-white/5 text-slate-200 hover:bg-white/10"><Link href={`/profile/${id}`}>View profile</Link></Button>{r?.status === "accepted" ? <Button variant="outline" disabled className="border-emerald-400/20 text-emerald-300"><UserCheck className="mr-1 h-4 w-4" />Connected</Button> : pendingFromMe ? <Button variant="outline" disabled className="border-amber-400/20 text-amber-300"><Clock3 className="mr-1 h-4 w-4" />Request sent</Button> : pendingToMe ? <><Button onClick={() => { const found = r?.id || r?._id; if (found) void respond(idOf(found), "accepted") }} disabled={busy === id} className="bg-gradient-to-r from-cyan-500 to-violet-600 text-white"><Check className="mr-1 h-4 w-4" />Accept</Button><Button onClick={() => { const found = r?.id || r?._id; if (found) void respond(idOf(found), "rejected") }} disabled={busy === id} variant="outline" className="border-white/10 text-slate-300"><X className="mr-1 h-4 w-4" />Decline</Button></> : <Button onClick={() => void connect(id)} disabled={busy === id} className="bg-gradient-to-r from-cyan-500 to-violet-600 text-white"><UserPlus className="mr-1 h-4 w-4" />Connect</Button>}</div></CardContent></Card>})}</div>}
      </>}

      {tab === "requests" && <div className="mt-5 space-y-3">{incoming.length === 0 ? <Card className="border-white/10 bg-white/[0.04]"><CardContent className="py-16 text-center text-slate-500">You have no pending connection requests.</CardContent></Card> : incoming.map(c => { const person = c.requester; const id = idOf(c._id || c.id); return <Card key={id} className="border-white/10 bg-white/[0.04]"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Avatar className="h-12 w-12"><AvatarImage src={assetUrl(person?.profileImage)} /><AvatarFallback>{person?.firstName?.[0] || "U"}</AvatarFallback></Avatar><div><p className="font-semibold">{person?.firstName} {person?.lastName}</p><p className="text-sm text-slate-500">@{person?.username || "developer"}</p></div></div><div className="flex gap-2"><Button onClick={() => void respond(id, "accepted")} disabled={busy === id} className="bg-gradient-to-r from-cyan-500 to-violet-600 text-white"><Check className="mr-1 h-4 w-4" />Accept</Button><Button variant="outline" onClick={() => void respond(id, "rejected")} disabled={busy === id} className="border-white/10"><X className="mr-1 h-4 w-4" />Decline</Button></div></CardContent></Card>})}</div>}
