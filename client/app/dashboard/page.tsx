...
import { Bell, Code2, Compass, Eye, FolderKanban, LogOut, MessageCircle, Plus, RefreshCw, Search, Sparkles, Users, BriefcaseBusiness, ArrowUpRight } from "lucide-react"
    ...
    <main className="relative mx-auto max-w-7xl px-4 py-7">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)_270px]">
        ...
        <section className="min-w-0 space-y-5">
          ...
          {filtered.map(post => { const author = post.author || post.user || {}; const liked = Array.isArray(post.likes) && post.likes.some((like: any) => String(like?._id || like?.id || like?.user?._id || like?.user) === String(user?.id)); return <article key={post._id} className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-start gap-3"><Avatar className="h-10 w-10"><AvatarImage src={assetUrl(author.profileImage)} /><AvatarFallback>{`${author.firstName?.[0] || ""}${author.lastName?.[0] || author.username?.[0] || "D"}`.toUpperCase()}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="font-semibold">{author.firstName || author.username || "Developer"} {author.lastName || ""}</p><p className="text-xs text-muted-foreground">{author.username ? `@${author.username}` : "DevHeaven member"}</p></div></div><h2 className="mt-4 text-lg font-bold">{post.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{post.content || post.body}</p><div className="mt-4 flex items-center gap-2 border-t border-border pt-3"><Button variant="ghost" size="sm" onClick={() => void like(post._id)} className={liked ? "text-pink-500" : "text-muted-foreground"}>Like {Array.isArray(post.likes) ? post.likes.length : 0}</Button><Link href={`/community/${post._id}`}><Button variant="ghost" size="sm" className="text-muted-foreground">Comments {Array.isArray(post.comments) ? post.comments.length : 0}</Button></Link></div></article> })}
        </section>
