...
import { Bell, Code2, Compass, Eye, FolderKanban, LogOut, MessageCircle, Plus, RefreshCw, Search, Sparkles, Users, BriefcaseBusiness, ArrowUpRight, Heart, Repeat2, Share2 } from "lucide-react"
...
type Post = { _id: string; title?: string; content?: string; body?: string; author?: Person; user?: Person; likes?: any[]; comments?: any[]; reposts?: any[]; repostOf?: { _id?: string; author?: Person } | null; createdAt?: string }
    ...
    <main className="relative mx-auto max-w-7xl px-4 py-7"><section className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-cyan-500/15 via-card to-violet-600/15 p-6 shadow-2xl shadow-black/10 md:p-8"><div className="absolute right-[-5rem] top-[-7rem] h-56 w-56 rounded-full bg-violet-500/20 blur-3xl" /><div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs font-semibold text-cyan-600 dark:text-cyan-300"><Sparkles className="h-3.5 w-3.5" /> Developer workspace</div><h1 className="text-3xl font-extrabold tracking-tight md:text-4xl">Good to see you, {user?.firstName || "Developer"}.</h1><p className="mt-2 max-w-2xl text-muted-foreground">Build your reputation, discover opportunities and connect with people who are building the future.</p></div><Button onClick={() => setShowComposer(true)} className="shrink-0 bg-gradient-to-r from-cyan-500 to-violet-600 font-semibold text-white shadow-lg shadow-violet-600/20 hover:opacity-90"><Plus className="mr-2 h-4 w-4" />Create a post</Button></div></section>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)_270px]"><aside className="hidden lg:block"><div className="sticky top-24 space-y-4"><div className="rounded-2xl border border-border bg-card p-4"><Link href={`/profile/${user?.id}`} className="flex items-center gap-3 rounded-xl p-2 hover:bg-muted"><Avatar className="h-11 w-11 border border-border"><AvatarImage src={assetUrl(user?.profileImage)} /><AvatarFallback className="bg-gradient-to-br from-cyan-500 to-violet-600 text-white">{initials}</AvatarFallback></Avatar><div className="min-w-0"><p className="truncate font-semibold">{user?.firstName} {user?.lastName}</p><p className="truncate text-xs text-muted-foreground">@{user?.username}</p></div></Link><div className="mt-4 grid gap-1"><Link href="/discovery" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><Compass className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />Discover developers</Link><Link href="/projects" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><FolderKanban className="h-4 w-4 text-violet-500 dark:text-violet-400" />My projects</Link><Link href="/jobs" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><BriefcaseBusiness className="h-4 w-4 text-amber-500 dark:text-amber-400" />Find opportunities</Link><Link href="/notifications" className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><span className="flex items-center gap-3"><Bell className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />Notifications</span>{unread > 0 && <Badge className="h-5 min-w-5 px-1 text-[10px]">{unread > 99 ? "99+" : unread}</Badge>}</Link></div></div><div className="rounded-2xl border border-border bg-gradient-to-br from-cyan-500/10 to-violet-500/10 p-4"><Sparkles className="h-5 w-5 text-cyan-500 dark:text-cyan-300" /><p className="mt-3 text-sm font-semibold">Make your profile stand out</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Add projects, skills and a strong introduction to help the right people find you.</p><Link href={`/profile/${user?.id}`} className="mt-3 inline-flex items-center text-xs font-semibold text-cyan-600 dark:text-cyan-300">Edit profile <ArrowUpRight className="ml-1 h-3 w-3" /></Link></div></div></aside>
        ...
        <section className="min-w-0 space-y-5"><div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3"><Search className="ml-2 h-4 w-4 text-muted-foreground" /><Input className="border-0 bg-transparent text-foreground placeholder:text-muted-foreground focus-visible:ring-0" placeholder="Search posts, developers or ideas..." value={query} onChange={e => setQuery(e.target.value)} /></div>
          ...
          {error && <div className="rounded-xl border border-red-400/20 bg-red-400/5 p-4 text-sm text-red-600 dark:text-red-300">{error}<button className="ml-3 font-semibold underline" onClick={() => void load()}>Retry</button></div>}
          {filtered.length === 0 && !error && <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><p className="font-semibold">No posts found</p><p className="mt-1 text-sm text-muted-foreground">Try another search or be the first to share something.</p></div>}
          {filtered.map(post => { const author = post.author || post.user || {}; const liked = Array.isArray(post.likes) && post.likes.some((like: any) => idOf(like) === String(user?.id)); const reposted = Array.isArray(post.reposts) && post.reposts.some((item: any) => idOf(item) === String(user?.id)); const originalAuthor = post.repostOf?.author; return <article key={post._id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">{post.repostOf && <div className="mb-3 flex items-center gap-2 text-xs font-medium text-muted-foreground"><Repeat2 className="h-3.5 w-3.5 text-emerald-500" />{author.firstName || author.username || "Someone"} reposted this</div>}<div className="flex items-start gap-3"><Avatar className="h-10 w-10"><AvatarImage src={assetUrl(author.profileImage)} /><AvatarFallback>{`${author.firstName?.[0] || ""}${author.lastName?.[0] || author.username?.[0] || "D"}`.toUpperCase()}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="font-semibold">{author.firstName || author.username || "Developer"} {author.lastName || ""}</p><p className="text-xs text-muted-foreground">{author.username ? `@${author.username}` : "DevHeaven member"}</p>{originalAuthor && <p className="mt-1 text-xs text-muted-foreground">Originally posted by {originalAuthor.firstName || originalAuthor.username || "Developer"} {originalAuthor.lastName || ""}</p>}</div></div><h2 className="mt-4 text-lg font-bold">{post.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{post.content || post.body}</p><div className="mt-4 flex flex-wrap items-center gap-1 border-t border-border pt-3"><Button variant="ghost" size="sm" disabled={busyPost === post._id} onClick={() => void like(post._id)} className={liked ? "text-pink-500" : "text-muted-foreground"}><Heart className={`mr-1.5 h-4 w-4 ${liked ? "fill-current" : ""}`} />{Array.isArray(post.likes) ? post.likes.length : 0}</Button><Link href={`/community/${post._id}`}><Button variant="ghost" size="sm" className="text-muted-foreground"><MessageCircle className="mr-1.5 h-4 w-4" />{Array.isArray(post.comments) ? post.comments.length : 0}</Button></Link><Button variant="ghost" size="sm" disabled={busyPost === post._id} onClick={() => void repost(post._id)} className={reposted ? "text-emerald-500" : "text-muted-foreground"}><Repeat2 className="mr-1.5 h-4 w-4" />{reposted ? "Reposted" : "Repost"}{Array.isArray(post.reposts) && post.reposts.length > 0 ? ` ${post.reposts.length}` : ""}</Button><Button variant="ghost" size="sm" onClick={() => void share(post)} className="text-muted-foreground"><Share2 className="mr-1.5 h-4 w-4" />Share</Button></div></article> })}
        </section>

        <aside className="hidden lg:block"><div className="sticky top-24 space-y-4"><div className="rounded-2xl border border-border bg-card p-5"><div className="flex items-center justify-between"><h3 className="font-semibold">Quick actions</h3><Sparkles className="h-4 w-4 text-cyan-500" /></div><div className="mt-4 grid gap-2"><Link href="/network" className="rounded-xl bg-muted px-3 py-2.5 text-sm font-medium hover:bg-muted/70">Grow your network</Link><Link href="/applications" className="rounded-xl bg-muted px-3 py-2.5 text-sm font-medium hover:bg-muted/70">Track applications</Link><Link href="/messages" className="rounded-xl bg-muted px-3 py-2.5 text-sm font-medium hover:bg-muted/70">Open messages</Link></div></div></div></aside>
      </div>
    </main>
  </div>
}
