  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"><div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <aside className="lg:col-span-1">{showNotifications && <Card className="mb-6"><CardContent className="pt-6 text-sm text-gray-600">You&apos;ll see notifications here as real activity occurs.</CardContent></Card>}<Card className="mb-6" id="create-post"><CardHeader><div className="flex items-center space-x-3"><Avatar className="h-12 w-12"><AvatarImage src={profileImage || "/placeholder.svg"} /><AvatarFallback>{initials}</AvatarFallback></Avatar><div><h3 className="font-semibold">{user.firstName} {user.lastName}</h3><p className="text-sm text-gray-600">{user.location || "Developer"}</p></div></div></CardHeader><CardContent><div className="space-y-2 text-sm"><div className="flex justify-between"><span className="text-gray-600">Connections</span><ConnectionStatus compact /></div><div className="flex justify-between"><span className="text-gray-600">Projects</span><span className="font-medium">{projectCount}</span></div><div className="flex justify-between"><span className="text-gray-600">Profile Views</span><span className="font-medium">0</span></div></div></CardContent></Card></aside>
        <main className="lg:col-span-2"><Card className="mb-6"><CardContent className="pt-6"><form onSubmit={handleCreatePost}><div className="flex space-x-3"><Avatar className="h-10 w-10"><AvatarImage src={profileImage || "/placeholder.svg"} /><AvatarFallback>{initials}</AvatarFallback></Avatar><div className="flex-1"><Input placeholder="Post title..." value={newPost.title} onChange={(e) => setNewPost((prev) => ({ ...prev, title: e.target.value }))} className="mb-3" required /><Textarea placeholder="Share your thoughts, projects, or ask questions..." className="min-h-[100px] resize-none" value={newPost.content} onChange={(e) => setNewPost((prev) => ({ ...prev, content: e.target.value }))} required /><div className="flex justify-end mt-4"><Button type="submit" className="bg-purple-600 hover:bg-purple-700" disabled={loading || !newPost.title.trim() || !newPost.content.trim()}>{loading ? "Posting..." : "Post"}</Button></div></div></div></form>{error && <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 mt-3 text-sm text-yellow-700">{error}</div>}</CardContent></Card><div className="space-y-6">{visiblePosts.length === 0 ? <Card><CardContent className="pt-6 text-center text-gray-500"><p>{searchTerm ? "No posts match your search yet." : "No posts yet. Be the first to share something!"}</p></CardContent></Card> : visiblePosts.map((post: any) => <Card key={post._id}><CardContent className="pt-6"><div className="flex space-x-3"><Avatar className="h-10 w-10"><AvatarImage src={assetUrl(post.author?.profileImage) || "/placeholder.svg"} /><AvatarFallback>{post.author?.firstName?.[0]}{post.author?.lastName?.[0]}</AvatarFallback></Avatar><div className="flex-1"><div className="flex items-center space-x-2 mb-2"><h4 className="font-semibold">{post.author?.firstName} {post.author?.lastName}</h4><span className="text-gray-500 text-sm">@{post.author?.username}</span><span className="text-gray-400 text-sm">•</span><span className="text-gray-500 text-sm">{new Date(post.createdAt).toLocaleDateString()}</span></div><h3 className="font-medium mb-2">{post.title}</h3><p className="text-gray-800 mb-3">{post.content}</p>{post.tags?.length > 0 && <div className="flex flex-wrap gap-2 mb-4">{post.tags.map((tag: string) => <Badge key={tag} variant="secondary" className="text-xs">#{tag}</Badge>)}</div>}<div className="flex items-center space-x-6 text-gray-500"><button className="flex items-center space-x-2 hover:text-red-500" onClick={() => handleLike(post._id)}><Heart className="h-4 w-4" /><span className="text-sm">{post.likes?.length || 0}</span></button><button className="flex items-center space-x-2 hover:text-blue-500" onClick={() => handleComment(post._id)}><MessageCircle className="h-4 w-4" /><span className="text-sm">{post.comments?.length || 0}</span></button><button className="flex items-center space-x-2 hover:text-green-500" onClick={() => handleShare(post._id)}><Share2 className="h-4 w-4" /><span className="text-sm">Share</span></button></div></div></div></CardContent></Card>)}</div></main>
