...
export async function fetchPosts(page: number | string = 1, limit = 20) {
  const normalizedPage = typeof page === "number" && Number.isFinite(page) && page > 0 ? page : 1;
  const normalizedLimit = Number.isFinite(limit) && limit > 0 ? Math.min(Math.floor(limit), 100) : 20;
  const body = await request(`/api/posts?page=${normalizedPage}&limit=${normalizedLimit}`);
