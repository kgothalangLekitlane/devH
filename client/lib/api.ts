...
async function request(path: string, options: RequestInit = {}) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw new Error("Unable to reach the DevHeaven API. Please check your connection and try again.");
    }
    throw error;
  } finally {
...
export async function fetchNotificationPreferences(token: string) { return request("/api/users/me/notifications", { headers: authHeaders(token) }); }
export async function updateNotificationPreferences(messages: boolean, token: string) { return request("/api/users/me/notifications", { method: "PATCH", headers: { ...authHeaders(token), "Content-Type": "application/json" }, body: JSON.stringify({ emailNotifications: { messages } }) }); }
...
export async function createPost(data: { title: string; content: string; tags?: string[]; media?: File[] }, token: string) {\n  const form = new FormData();\n  form.append("title", data.title);\n  form.append("content", data.content);\n  if (data.tags?.length) form.append("tags", JSON.stringify(data.tags));\n  for (const file of data.media || []) form.append("media", file);\n  return request("/api/posts", { method: "POST", headers: authHeaders(token), body: form });\n}
